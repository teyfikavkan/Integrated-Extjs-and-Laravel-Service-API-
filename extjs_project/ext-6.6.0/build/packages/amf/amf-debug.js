...
Ext.define('Ext.data.amf.Encoder', {alias:'data.amf.Encoder', config:{format:3}, bytes:[], constructor:function(config) {
  ...
  var bias = (1 << ebits - 1) - 1, s, e, f, ln, i, bits, str, data = [];
  var K_INFINITY = [127, 240, 0, 0, 0, 0, 0, 0], K_NINFINITY = [255, 240, 0, 0, 0, 0, 0, 0], K_NAN = [255, 248, 0, 0, 0, 0, 0, 0];
  if (isNaN(num)) {
    data = K_NAN;
  } else {
    if (num === Infinity) {
...
Ext.define('Ext.data.amf.XmlEncoder', {alias:'data.amf.xmlencoder', body:'', statics:{generateFlexUID:function(id) {
  ...
  var ordinals = [], firstNonOrdinal, ecmaElements = [], length = array.length, i, str;
  for (i in array) {
  ...
  var msg, str;
  str = '\x3camfx ver\x3d"3" xmlns\x3d"http://www.macromedia.com/2005/amfx"\x3e\x3cbody\x3e';
  str += message.encodeMessage();
  str += '\x3c/body\x3e\x3c/amfx\x3e';
  return str;
...
Ext.define('Ext.direct.AmfRemotingProvider', {alias:'direct.amfremotingprovider', extend:'Ext.direct.Provider', requires:['Ext.util.MixedCollection', 'Ext.util.DelayedTask', 'Ext.direct.Transaction', 'Ext.direct.RemotingMethod', 'Ext.data.amf.XmlEncoder', 'Ext.data.amf.XmlDecoder', 'Ext.data.amf.Encoder', 'Ext.data.amf.Packet', 'Ext.data.amf.RemotingMessage', 'Ext.direct.ExceptionEvent'], binary:false, maxRetries:1, timeout:undefined, constructor:function(config) {
  ...
  var me = this, request = {url:me.url, callback:me.onData, scope:me, transaction:data, timeout:me.timeout}, callData, i = 0, len, params, encoder, amfMessages = [], amfHeaders = [];
  if (Ext.isArray(data)) {
    if (!me.binary) {
  ...
  var data = null, rawBytes = [], events = [], event, i = 0, len, decoder;
  try {
    if (this.binary) {
